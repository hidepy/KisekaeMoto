...
export default {
  ...
  data: function() {
    return {
      ...
      adjustRatio:
        ($("#moto-image-gn125-front").width() || document.body.clientWidth) /
        1280
    };
  },
  ...
  mounted: function() {
    // 選択中のバイクを更新
    this.selectedMotoName =
      localStorage[LOCAL_STORAGE_KEYS.SELECTED_MOTO_NAME] || "GN125";
  ...
  methods: {
    ...
    updateImgDef: function() {
      ...
      setTimeout(() => {
        ...
        imgs.on("error", error => {
          ...
          M.toast({
            html:
              "画像のロードに失敗...ページをリフレッシュして下さい...(" +
              $(error.target).attr("id") +
              ")"
          });
        });
    ...
    refreshAllParts: function() {
      ...
      const maxImgHeight = (window.innerHeight * 4) / 3 - 1 - 80;
        ...
        .map(v => {
          return {
            target: document.querySelector(
              `#controll-panel-${v.fileName}-hue-rotate`
            )
          };
        })
        .forEach(v => {
    ...
    updateParts: function(event) {
      ...
      // idを基に、部品に紐づく設定情報を取得
      $(`#${wrapperId} .controll-panel-input`).each((i, v) => {
        const localControllName = $(v).attr("settingname");

        const val = `${localControllName}(${$(v).val()}${
          unitMap[localControllName]
        })`;
